...
class Section{
    ...
    friend ostream& operator<<(ostream& os, const StudentRecord& randomRecord);
    friend ostream& operator<<(ostream& os, const TimeSlot& randomTime);
public:
    ...
    bool addGradeSection(const string& studentName, int grade, int labNumber){
...
private:
    // StudentRecord class nested in the Section class
    class StudentRecord{
        friend ostream& operator<<(ostream& os, const StudentRecord& randomRecord);
    public:
    ...
    // TimeSlot class nested in the Section class
    class TimeSlot{
        friend ostream& operator<<(ostream& os, const TimeSlot& randomTime);
    public:
...
public:
    ...
    bool addGrade(const string& studentName, int grade, int labNumber){
       return sec -> addGradeSection(studentName, grade, labNumber);
    }
private:
...
ostream& operator<<(ostream& os, const Section& randomSection){
    os << "Section: " << randomSection.sectionName << randomSection.theTiming;
    ...
    if(randomSection.recordsOfGrades.size() == 0){
        ...
        os << endl;
        for(const Section::StudentRecord* currStudent : randomSection.recordsOfGrades){
            os << *currStudent;
        }
    }
    return os;
...
// overloading the operator for TimeSlot class
ostream& operator<<(ostream& os, const Section::TimeSlot& randomTime){
    os << ", Time Slot: [Day: " << randomTime.day << ", Start time: ";
    // input is in 24 hour time so we must convert to 12 hour clock with the appropriate ending
    if(randomTime.timing > 12){
        os << randomTime.timing - 12 << "pm], Students: ";
    }
    else{
        os << randomTime.timing << "am], Students: ";
    }
    return os;
}

// overloading the operator for StudentRecord class
ostream& operator<<(ostream& os, const Section::StudentRecord& randomRecord){
    os << "Name: " <<  randomRecord.studentName << ", Grades:";
    for(int currGrade : randomRecord.studentGrades){
        os << " " << currGrade;
    }
    os << endl;
    return os;
