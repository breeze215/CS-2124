...
const Node* isSublist(const Node* pattern, const Node* target){
    ...
    while(curr){
        while(curr && curr -> data != pattern -> data){
            curr = curr -> next;
        }
        if(!curr){
            return nullptr;
        }
        const Node* startNode = curr;
        const Node* subNode = pattern;
        while(subNode && curr && subNode -> data == curr -> data){
...
const Node* sharedListUsingSet(const Node* listA, const Node* listB){
    unordered_set<const Node*> adresses;
    while(listA){
        adresses.insert(listA);
        listA = listA -> next;
    }
    while(listB){
        if(adresses.find(listB) == adresses.end()){
            listB = listB -> next;
        }
        else{
            return listB;
