...
public:
    ...
    public:
        Iterator(Node* pointer) : currNode(pointer) {}
        Iterator& operator++(){
    ...
    public:
        Const_Iterator(Node* pointer) : currNode(pointer) {}
    ...
    Iterator insert(const Iterator& nextIter, int data){
        Node* next = nextIter.currNode;
        next -> prior = new Node(data, next -> prior, next);
        next -> prior -> prior -> next = next -> prior;
        ++theSize;
        return Iterator(next -> prior);
    }
