...
public:
   ...
   const bool getHired() const { return hired; }
...
public:
   ...
   const double totalArmyStrength() const{
    ...
    const size_t getNobleArmySize() const { return nobleArmy.size(); }
   ...
   void updateNobleArmyStrength(double winner, double loser){
    ...
    //getters
   const bool getStatus() const { return alive; }
...
void deletePointers(vector <Warrior*>& warriors, vector<Noble*> nobles){
    ...
    for(const Noble* currNoble : nobles){
        delete currNoble;
    }
}
