...
public:
    ...
    Warrior(const string& nameOfWarrior, int strengthOfWarrior) : name(nameOfWarrior), strength(strengthOfWarrior), hired(false) {}
...
public:
    ...
    const double& totalArmyStrength() const{
    ...
    void updateNobleArmyStrength(double winnerStrength, double loserStrength){
        for(Warrior*& currWarrior : nobleArmy){
            double newStrength = (currWarrior -> getStrength()) * ((winnerStrength - loserStrength) / winnerStrength);
    ...
    void die(){
        alive = false;
        for(Warrior*& currWarrior : nobleArmy) { currWarrior -> setStrength(0); }
    }
    
    // function to hire a warrior
    bool hire(Warrior& randomWarrior){
        if(!alive || randomWarrior.getHired()){
            // fail if noble is dead or warrior is already hired
            cout << name << " failed to hire " << randomWarrior.getName() << endl;
            return false;
        }
        else{
            randomWarrior.setHired();
            nobleArmy.push_back(&randomWarrior);
            return true;
        }
    }
    
    // function to fire a warrior
    bool fire(Warrior& randomWarrior){
        if(!alive){
            // fail to fire a warrior if the noble is dead
            cout << name << " has failed to fire " << randomWarrior.getName() << endl;
            return false;
        }
        bool fired = false;
        for(size_t index = 0; index < nobleArmy.size(); index++ ){
            if(fired == false){
                if(nobleArmy[index] == &randomWarrior){
                    cout << randomWarrior.getName() << ", you don't work for me anymore! -- " << name << "." << endl;
                    randomWarrior.setHired();
                    fired = true;
                }
            }
        ...
        if(fired){
            ...
            cout << name << " failed to fire " << randomWarrior.getName() << endl;
        }
        return fired;
    }
    ...
    void battle(Noble& enemyNoble){
        cout << name << " battles " << enemyNoble.name << endl;
        if(!alive && !enemyNoble.alive){
            cout << "Oh, NO! They're both dead! Yuck!" << endl;
        }
        else if(!alive && enemyNoble.alive){
            cout << "He's dead, " << enemyNoble.name << endl;
        }
        else if(alive && !enemyNoble.alive){
            ...
            double enemyStrength = enemyNoble.totalArmyStrength();
            if(myStrength == enemyStrength){
                die();
                enemyNoble.die();
                cout << "Mutual Annihilation: " << name << " and " << enemyNoble.name << " die at each other's hands" << endl;
            }
            else if(myStrength > enemyStrength){
                enemyNoble.die();
                updateNobleArmyStrength(myStrength, enemyStrength);
                cout << name << " defeats " << enemyNoble.name << endl;
            }
            else{ // enemyStrength > myStrength
                die();
                enemyNoble.updateNobleArmyStrength(enemyStrength, myStrength);
                cout << enemyNoble.name << " defeats " << name << endl;
            }
        }
    }
...
ostream& operator<<(ostream& os, const Noble& randomNoble){
    ...
    for(Warrior* currWarrior : randomNoble.nobleArmy){
        if(currWarrior -> getStrength() >= 0){
            os << *currWarrior << endl;
        }
    }
    return os;
