...
int main() {
    ...
    while(warriorStream >> command){
        ...
        if(command == "Warrior"){
            ...
            // construct a new Warrior and put it at the end of the warriorVector only if it is not already in the warriorVector
            size_t checkIfExists = searchWarrior(name, warriorVector);
            if(checkIfExists == warriorVector.size()){
